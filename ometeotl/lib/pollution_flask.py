...
app = Flask(__name__)
CORS(app, resources={r"/get_pollution_prediction": {"origins": "*"}})

@app.route('/get_pollution_prediction', methods=['POST'])
def get_pollution_prediction():
    try:
        ...
        if response.status_code == 200:
            ...
            with open('ModeloCont.pk1', 'rb') as model_file:  # Change the path to your model file
            ...
            response_data = {
                ...
                'result': result.tolist()  # Convert result to a list
            }
            print(response_data)

            return jsonify(response_data), 200
        else:
            return jsonify({'error': f"Request failed {response.status_code}"}), 400
